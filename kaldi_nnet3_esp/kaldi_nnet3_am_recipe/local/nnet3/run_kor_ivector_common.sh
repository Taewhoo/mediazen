...
stage=0
nj=30
train_set=trainset   # you might set this to e.g. train.
test_sets=testset
...
if [ $stage -le 2 ]; then
  ...
  if $use_pitch; then
    ...
    steps/make_mfcc_pitch.sh --pitch-config conf/pitch.conf --nj $nj --cmd "$train_cmd" data/${train_set}_sp || exit 1;
  else
    steps/make_mfcc.sh --nj $nj --cmd "$train_cmd" data/${train_set}_sp || exit 1;
  fi
