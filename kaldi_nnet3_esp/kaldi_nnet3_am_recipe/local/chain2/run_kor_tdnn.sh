...
if [ $stage -le 22 ]; then
  ...
  steps/chain2/train.sh \
    ...
    --num-jobs-initial $ngpu_initial --num-jobs-final $ngpu_final \
...
if $test_online_decoding && [ $stage -le 25 ]; then
  ...
  for data in $test_sets; do
    (
      ...
      # cer scroing
      steps/scoring/score_kaldi_cer.sh --cmd "$decode_cmd" data/${data} $tree_dir/graph ${dir}_online/decode_${data_affix}
    ) || touch $dir/.error &
  done
  wait
